...
async fn run_native_tool(
    ...
    // Tool's declared access, resolved here so the permission manager does not
    // need its own lookup into the tool registry.
    let access = entry
        .tool
        .required_permission()
...
async fn execute_mcp_tool(
    ...
    if let Err(e) = ctx
        ...
        .enforce(
            ...
            &ctx.cancel,
            ctx.mode.plan_path(),
            // MCP scopes are JSON blobs, not paths, so nothing to override.
            None,
        )
        .await
    {
